...
def product_derive(mode):
    ...
    if mode == "dev":
        ...
        deploy_compose = os.path.join(product_dir, "docker", "docker-compose.deploy.yml")
        ...
        if compose_to_check:
            ...
            click.echo("        \u2139\ufe0f  No docker-compose.deploy.yml yet — will be generated by product:build.")
    ...
    if not port_conflicts:
        ...
        if mode == "dev" and "prod" in blocking_envs_flat:
            click.echo("        💡 Tip: Run 'splent product:down --prod' to stop the production deployment first.")
        elif mode == "prod" and "dev" in blocking_envs_flat:
            click.echo("        💡 Tip: Run 'splent product:down --dev' to stop the development environment first.")

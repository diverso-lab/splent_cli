...
def db_rollback(feature, steps):
    ...
    click.echo(click.style(f"  ⬇️  Rolling back {steps} step(s) for {feature}...", fg="cyan"))
    try:
        alembic_downgrade(directory=mdir, revision=f"-{steps}")
        revision = MigrationManager.get_current_feature_revision(feature, app.extensions["migrate"].db.engine)
